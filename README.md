...
正式发布时建议补录一条完整新人链路，使用脱敏测试剧本和测试账号：

-   **0–8 秒｜登录与进入工作台**：展示登录页，登录后进入工作台。

-   **8–22 秒｜导入剧本**：填写剧本名称，上传脱敏测试剧本或粘贴文本。

-   **22–45 秒｜八节点分析**：点击开始分析，展示八节点执行状态；等待过程可适当加速。
...
使用者可以在管理后台编写自己的评估体系，或将系统连接至自行选择的大模型服务。调用第三方模型产生的 Token 费用由使用者与对应服务商结算。
...
> 默认账号仅用于首次本地体验。首次登录后请立即修改密码。禁止使用默认密码将系统直接部署至公网环境。
...
如发现安全漏洞，请勿在公开 Issue 中披露漏洞细节。请发送邮件至 contact@wy\.cn，并提供影响范围、复现步骤和必要的脱敏信息。
